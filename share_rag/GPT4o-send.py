...
def faultlocalization(prompt_location, code_location, res_path, db_path, files_loc, experiment_model, faultdata, type,
    ...
    # RAG检索结果
    if type == 1:
        rrf_res = RAG(files_loc, db_path, res_path, code_location, db)
...
def run_one_tutorcode(
    ...
    try:
        ...
        if experiment_model == "deepseek-ai/DeepSeek-R1":
            res_path = os.path.join(root_path, version_str, "deepseek-r1", str(experiment_index))
        else:
            res_path = os.path.join(root_path, version_str, experiment_model, str(experiment_index))

        os.makedirs(res_path, exist_ok=True)

        files_loc = "/home/weijiqing/miniconda3/envs/llmfl/LLMFL/Data/TutorCode_train.txt"
...
def test_Code(prompt_location, experiment_index, experiment_model, rangeIndex, type, db):
    root_path = "/home/weijiqing/miniconda3/envs/llmfl/LLMFL/dataset/TutorCode_Test/"
    success_versions = []
    ...
    max_workers = 2  # ✅ 并发数，建议 4~8，Claude 很容易限流
...
def run_all(prompt_location):
    # 批量跑实验
    experiment_model = "deepseek-v3.1"
    dataset = 1
    for i in [1]:
        test_Code(prompt_location, 0, experiment_model, 9999, 0, dataset)
        test_Code(prompt_location, 1, experiment_model, 9999, 1, dataset)
        test_Code(prompt_location, 2, experiment_model, 9999, 2, dataset)
