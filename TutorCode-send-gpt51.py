...
from utils.utils1 import adaptive_RAG, calculate_metrics,get_ground_truth_id # 引入新函数
...
def faultlocalization(prompt_location, code_location, res_path, db_path, files_loc, experiment_model, faultdata, type,
    ...
    #     prompt = build_oneshot(prompt_model, code_location, rrf_res)
        ...
        rrf_res_text, retrieved_ids = adaptive_RAG(
            files_loc, db_path, res_path, code_location, db, experiment_model,0
        )
        prompt = build_oneshot(prompt_model, code_location, rrf_res_text)    
    else:
...
def test_Code(prompt_location, experiment_type, experiment_model, rangeIndex, type, db):
    ...
    success_versions = []
    filter_data_path = "evaluate/TutorCode_Filter_Data.pkl"
...
def run_all(prompt_location):
    # 批量跑实验
    experiment_model = "gpt-5.1"
    for i in [1]:
        # test_Code(prompt_location, 0, experiment_model, 9999, 0, 1)
        # test_Code(prompt_location, 1, experiment_model, 9999, 1, 1)
        # test_Code(prompt_location, 2, experiment_model, 9999, 2, 1)
        test_Code(prompt_location, 3, experiment_model, 9999, 2, 1)
